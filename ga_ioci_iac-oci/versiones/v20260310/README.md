...
```powershell
terraform plan -var-file .\oci-credentials.auto.tfvars.json -var-file .\giss_governance__v3.2.json -var-file .\giss_iam_v3.2.json -var-file .\giss_network_hub_b_empty_v3.2.json -out tfplan
```
...
```powershell
terraform apply -var-file .\oci-credentials.auto.tfvars.json -var-file .\giss_governance__v3.2.json -var-file .\giss_iam_v3.2.json -var-file .\giss_network_hub_b_empty_v3.2.json -out tfplan
```
...
Perfecto. A continuación agrego la nueva sección al README manteniendo  **exactamente el formato, estructura y estilo actual** , sin modificar ni eliminar absolutamente nada de lo anterior.

Puedes añadir esto como nueva sección **#16** al final del documento:

---

# 🚫 16. Troubleshooting: Entorno Corporativo con Restricción de Red (HashiCorp Providers)

---

## 📌 16.1 Problema Detectado

En entorno Windows 11 Enterprise (VDI corporativa), durante la ejecución:
...
Error while installing hashicorp/time v0.13.1:
releases.hashicorp.com: bad response code: 403
```

Prueba directa de conectividad:

```powershell
Invoke-WebRequest https://releases.hashicorp.com/
```

Resultado:

```
Access Denied / Noncompliant action
```
...
La red corporativa bloquea el acceso a:
...
Terraform intenta descargar providers desde dichos endpoints durante `terraform init`.
El bloqueo genera HTTP 403 y provoca el fallo de inicialización.
...
## 🛠 16.3 Solución Implementada: Provider Mirror Offline

Se implementó un esquema de instalación de providers completamente offline.

---

## 🔹 16.3.1 Generar Mirror en equipo con acceso a Internet
...
cd C:\oci-terraform\Giss-terraform
terraform init
terraform providers mirror C:\tmp\tf-mirror
```
...
## 🔹 16.3.2 Transferencia a VDI

Se copiaron vía OneDrive:

* Proyecto completo (incluyendo `.terraform.lock.hcl`)
* Carpeta `tf-mirror`

Hacia:

```
C:\oci-terraform\provider-cache
```

---

## 🔹 16.3.3 Configuración de Terraform para uso exclusivo del mirror
...
provider_installation {
  filesystem_mirror {
    path    = "C:\\oci-terraform\\provider-cache"
...
## 🔹 16.3.4 Inicialización en entorno restringido
...
✔ Providers instalados desde filesystem
✔ Sin conexión externa
✔ Inicialización exitosa

---

## ⚠ 16.4 Consideraciones Importantes

* No usar `-upgrade` en entornos restringidos.
* `.terraform.lock.hcl` es obligatorio al transferir proyectos.
* Copiar únicamente `.terraform/` no es suficiente.
* `terraform.rc` controla la política global de instalación.
* No se modificaron reglas de firewall ni se solicitó whitelist.

---

## 🧩 16.5 Alternativa Corporativa (No aplicada)
...
No fue necesario gracias al enfoque offline.

---

## ✅ 16.6 Estado Final

✔ Terraform operativo en VDI restringida
✔ Proyecto de producción funcional
✔ Sin dependencia de descarga externa de providers
✔ Sin impacto en infraestructura OCI
